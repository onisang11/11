...
class TestLayout:
    ...
    def test_weighted_spring_layout(self):
        ...
        same_mass = nx.drawing.fruchterman_reingold_layout(
            hyper, node_weight="node_weight", seed=1
        )

        hyper.nodes[(0, 0, 0, 0, 0)]["node_weight"] = 100
        dif_mass = nx.drawing.fruchterman_reingold_layout(
            hyper, node_weight="node_weight", seed=1
        )
        first_distance = np.linalg.norm(
            same_mass[(0, 0, 0, 0, 0)] - same_mass[(0, 0, 0, 0, 1)]
        )
        second_distance = np.linalg.norm(
            dif_mass[(0, 0, 0, 0, 0)] - dif_mass[(0, 0, 0, 0, 1)]
        )
